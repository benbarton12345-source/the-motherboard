...
| `performed_sessions` | id, session_id (FK→sessions, nullable for swaps/freestyle), programme_id (FK→programmes), performed_date, session_rating (int /10, nullable), energy_rating (int /10, nullable), session_note (nullable) |
| `performed_exercises` | id, performed_session_id (FK→performed_sessions, cascade), exercise_id (FK→exercises), sort_order, exercise_note (nullable) |
| `performed_sets` | id, performed_exercise_id (FK→performed_exercises, cascade), set_number, target_reps, actual_reps, actual_weight |
...
**Step 2 — Programme builder complete (21 June 2026).** `src/components/ProgrammeBuilderModal.jsx`. Tables: `programmes`, `sessions`, `session_exercises`, `session_sets` — all RLS disabled. Training tab shows live Sessions card with exercise counts per session.

**Note for step 3+:** the user's original training spreadsheet contains rep targets, working sets, tempo, rest times, and detailed coaching cues per exercise. These map onto programme targets (reps/rest) and potential per-exercise notes. Refer to that sheet when refining programmes or building the logging screen rather than re-entering data from scratch.

**Step 3 — Phone logging screen complete (22 June 2026).** `src/components/TrainingSession.jsx`. Full-screen flow: overview → set logging → finish. Saves to `performed_sessions`/`performed_exercises`/`performed_sets` (all RLS disabled).

Persistence is **save-as-you-go** (changed from save-at-end on 22 June 2026):
- `performed_sessions` row created on Start session (ratings/note null until Finish).
- `performed_exercises` rows created lazily — only when an exercise's first set is logged (or its note is saved), guarded by a ref so rapid taps can't double-insert. Skipped exercises never get a row.
- Each `performed_sets` row is inserted on first log and updated on edit; removing a set deletes its row; swapping an exercise deletes the old `performed_exercises` row (cascade clears its sets) and the new one starts fresh.
...
#### Productivity redesign — Session 3 of 3 complete (11 July 2026): Tasks + Reading + Weekly Review

Final session of the Productivity redesign. The section is now fully split into its four sub-pages (Overview, Habits & Goals, Tasks, Reading) plus the Weekly Review modal, with purple removed everywhere.

**Weekly Review modal** — `src/components/WeeklyReviewModal.jsx`. Six fields in the specified order, keyed on `weekly_reviews.week_start` (the table's real unique key — NOT `week_start_date`). Week navigator (‹ ›, capped at current week) for retrospective past-week entry; autosaves a draft on blur; **Seal Week** validates the consistency score (1–10 required), warns if all fields blank, then sets `sealed = true` + `sealed_at = now()` and the row becomes read-only. A **History** view lists past reviews (sealed show score + first line; unsealed past weeks show "Not completed"), click-through sets the navigator to that week. Wired into the Overview weekly-review card (edit + "View past reviews"), which refreshes card state + streak on seal. Uses the shared `Modal` component.

**Reading sub-page** — `src/components/ReadingPage.jsx` (thin wrapper around the existing `ReadingPanel`), routed at `reading`. **Purple fully removed** across `ReadingPanel`, `ReadingCard` (Home), and `readingHelpers.js`: the `READING_PURPLE` constant → `READING_ACCENT` (`#34d399`), the heatmap ramp → emerald, and the two purple genre hues remapped (Self-development → teal `#2dd4bf`; spare palette slot → orange `#fb923c`). New **"📖 Log 10 min reading"** action (`useReading.logReading`): bumps today's `reading_activity` intensity + the current book's progress (+5%) and marks the "10 mins reading" habit done for today (matched by name `/read/i && /min/i`) — the one hard-wired Reading→Habit cross-metric link (see below). 5-second **Undo** (`undoReadingLog`) reverts all three effects. Added inline **edit** to queued books (`updateBook`).

**Tasks sub-page** — `ProductivityPage.jsx` restructured (not replaced — all recurring/calendar behaviour preserved). Layout is now a full-width vertical stack: **Weekly Calendar** (full width, real-Date week nav verified across month boundaries — paging back from 6 Jul correctly shows 29 Jun) → **Overdue** (new; red card, 2-col grid, "N days overdue", complete/edit/delete) → **Today** (`TodaysTasks`) + **Upcoming** → **Completed** (new; collapsed, uncheck/delete) → **Recurring Tasks**. Removed from the page: the weekly-goals card, the week-summary card, and the embedded Reading panel. Recurring accent colour changed purple → **teal** (`teal-400`) in the calendar blocks and REC badges so recurring stays distinguishable from one-off (emerald) without purple.

Data decisions / notes:
- **`reading_activity` table created** — it was documented in BRIEF but never actually in the DB, so `useReading` had been reading/writing a non-existent table (heatmap/streak silently dead). `sql/reading_activity.sql` was written and run in Supabase on 11 July 2026. Shape unchanged from the doc: `activity_date` PK, `intensity`, `updated_at`; RLS disabled.
- **`weekly_goal_logs` retired in code** — no component references it any more (the Tasks page dropped its weekly-goals section; weekly goals live only on Habits & Goals via `weekly_goal_completions`). The legacy table + its historical rows are left in the DB untouched; it can be dropped whenever convenient.
- **Reading→Habit link** is implemented directly in `useReading.logReading` (find habit by name, insert `habit_completions` for today, undoable). This is the concrete pattern the future general `linked_source` engine should generalise: on a source event, look up subscribed goals/habits and apply the effect, capturing enough state to undo. No other cross-metric links were built.
- **Overview stubs already real** — the Session 2 Overview was already reading live `tasks` (today done/total + overdue) and `books` (current book + finished count), so no stub-wiring was needed; the overdue count matches the rebuilt Tasks page.

**Full Productivity section redesign is now complete** (Overview, Habits & Goals, Tasks, Reading, Weekly Review modal). Purple has been removed from the entire section (recurring items use teal; Reading uses emerald/teal).
...
- **Categorisation order:** Layer 1 exclusions/special handling (LIFENET→salary, DEFT→rent, LAURA/Transfer-from→reimbursement income, CASH DEPOSIT excluded, Amex bill payment on Commbank — `AMERICAN EXPRESS`/`\bAMEX\b`, scoped to `source==='commbank'` — excluded as an inter-account transfer since the purchases arrive via the Amex CSV, Transfer-to-Laura-by-reference→category, other positive Commbank→Section C) → Layer 2 `categoryRules.js` keywords (→ Section B) → Layer 3 AI fallback for the rest (→ Section C).
...
### Budgeting Insights & Analytics (build 2 of 2 — complete as of 5 July 2026)

`src/components/BudgetingInsights.jsx` — a two-tab analytics section (**Overview** / **Spending Intelligence**) rendered inside `FinancePage` **below the Monthly Budget block** (Section 4b), above Snapshot History. It fetches a 13-month `budget_entries` window + `statement_imports` itself; `snapshots` and a `reloadKey` (bumped after each import) are passed down from `FinancePage`. Shows an **empty state** whenever no `statement_imports` row exists for the selected month. Reads live data only — no hardcoded figures. Desktop-primary.

- **Pure calc in `src/utils/insightsData.js`** (no DB/React): category×month matrix, month totals, waterfall series, forecast-vs-actual, needs/wants, health score, unusual spend, subscription creep, one-off timeline, spending velocity, FI trajectory, what-if. `src/utils/insightsSettings.js` holds the savings-rate target + FI target/date.
